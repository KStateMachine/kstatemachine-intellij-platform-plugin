...
**The official IntelliJ Platform plugin for the [KStateMachine](https://github.com/KStateMachine/kstatemachine) Kotlin library**
...
> [!NOTE]
> The plugin is in early development. Feedback on real-world DSL usage is especially welcome — open an issue or drop into the [Slack channel](https://kotlinlang.slack.com/archives/C07DVAEKLM8).
...
<kbd>Settings/Preferences</kbd> → <kbd>Plugins</kbd> → <kbd>Marketplace</kbd> → search **"KStateMachine Visual"** → <kbd>Install</kbd>
...
1. Grab the latest `.zip` from the [GitHub Releases](https://github.com/KStateMachine/kstatemachine-intellij-platform-plugin/releases/latest) page, or from the [Marketplace versions page](https://plugins.jetbrains.com/plugin/MARKETPLACE_ID/versions).
2. In your IDE: <kbd>Settings/Preferences</kbd> → <kbd>Plugins</kbd> → <kbd>⚙️</kbd> → <kbd>Install plugin from disk…</kbd>
...
- [ ] Set the `MARKETPLACE_ID` in the badges above. You can obtain it once the plugin is published to JetBrains Marketplace.
